...
func (cc TaskService) UpdateTaskById(id string, userId string, payload dtos.UpdateTaskRequest) (data models.Task, findErr error, updateErr error) {
...
func (cc TaskService) DeleteTaskById(id string, userId string) (result *gorm.DB) {
	...
	*/
	...
	// Only that user who owns this task can update it's status
	return cc.db.DB.Where("id = ? AND user_id = ?", id, userId).Unscoped().Delete(&task)
}
